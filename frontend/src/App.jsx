...
function App() {
  ...
  useEffect(() => {
    (async () => { if (showLorebooks) { try { const lbs = await listLorebooks(); setLorebooks(lbs); } catch {} } })();
  }, [showLorebooks]);
        ...
        {showLorebooks && (
          <section className="characters">
            ...
            <div className="char-list">
              {lorebooks.map(lb => (
                <div key={lb.id} className="char-item">
                  <div>
                    <h3>{lb.name}</h3>
                    <div className="muted">{lb.description} — {lb.entry_ids?.length ?? 0} entries</div>
                  </div>
                </div>
              ))}
            </div>
